jQuery(document).ready(function() {
	...
	taxField.live('change', function () {
		tax = taxField.val();
        if(tax.length>0) {
            Dataprovider.ajaxCall('tax',tax,domain);
		}
	});
	...
	cocField.live('change', function () {
		coc = cocField.val();
        if(coc.length>0) {
            Dataprovider.ajaxCall('coc',coc,domain);
        }
	});
	...
	phoneField.live('change', function () {
        ...
        if(phone.length>0) {
            Dataprovider.ajaxCall('phone',phone,domain);
        }
	});
	...
	emailField.live('change', function () {
        ...
        if(email.length>0) {
            Dataprovider.ajaxCall('email',email,domain);
        }
	});
	...
	companyField.live('change', function () {
        ...
        if(company.length>0) {
            Dataprovider.ajaxCall('company',company,domain);
        }
	});
